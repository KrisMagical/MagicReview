...
def test_config_loader_discovers_yaml_and_json(tmp_path: Path) -> None:
    (tmp_path / "reviewagent.yml").write_text("rules:\n  no_select_star:\n    enabled: true\n    severity: high\n", encoding="utf-8")
    config = RuleConfigLoader().load(tmp_path)
    assert config.rules["no_select_star"]["enabled"] is True

    (tmp_path / "reviewagent.yml").unlink()
    (tmp_path / "reviewagent.json").write_text(json.dumps({"rules": {"max_parameters": {"enabled": True, "max_params": 2}}}), encoding="utf-8")
    config = RuleConfigLoader().load(tmp_path)
...
def test_config_loader_reports_yaml_syntax_and_invalid_severity(tmp_path: Path) -> None:
    bad_yaml = tmp_path / "reviewagent.yml"
...
def test_config_loader_uses_yaml_safe_load(tmp_path: Path) -> None:
    config_file = tmp_path / "reviewagent.yml"
